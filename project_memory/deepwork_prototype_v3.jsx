...
function Analytics({ sessions, onBack }) {
  ...
  // Build last 7 days with full Date objects
  const days = Array.from({ length: 7 }).map((_, i) => {
    const d = new Date();
    d.setDate(d.getDate() - (6 - i));
    d.setHours(0, 0, 0, 0);
    const label = d.toLocaleDateString("en-US", { weekday: "short" });
    const daySessions = sessions.filter(s => new Date(s.ts).toDateString() === d.toDateString());
    const total = daySessions.reduce((acc, s) => acc + s.duration, 0);
    return { label, total, date: d, count: daySessions.length };
  });
  ...
  // Summary stats — scoped to selected day or today
  const statDate = selectedDate || todayDate;
  const statSessions = sessions.filter(s => new Date(s.ts).toDateString() === statDate.toDateString());
  const statTotal = statSessions.reduce((acc, s) => acc + s.duration, 0);
  const statLabel = statDate.toDateString() === todayDate.toDateString()
    ? "Today"
    : statDate.toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" });

  // Filtered + sorted log
  const filteredSessions = (selectedDate
    ? sessions.filter(s => new Date(s.ts).toDateString() === selectedDate.toDateString())
    : sessions
  ...
  const handleDayClick = (date) => {
    if (selectedDate && selectedDate.toDateString() === date.toDateString()) {
      setSelectedDate(null); // deselect = show all
    } else {
  ...
  // Tag breakdown — scoped to filter if active
  const byTag = filteredSessions.reduce((acc, s) => {
      ...
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 48 }}>
          <button onClick={onBack} style={{
            ...
            fontFamily: "var(--mono)", fontSize: 11, letterSpacing: "0.1em", transition: "all 0.2s",
          }}
        ...
        {/* summary stats — reactive to selected day */}
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            {selectedDate && (
              <button onClick={() => { setSelectedDate(null); setPage(1); }} style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 40 }}>
            {[
              { label: "Focus Time", value: fmtDuration(statTotal) },
              { label: "Sessions", value: statSessions.length },
              { label: "Avg Session", value: statSessions.length ? fmtDuration(Math.round(statTotal / statSessions.length)) : "—" },
            ].map(({ label, value }) => (
              <div key={label} style={{
                background: "var(--surface)", border: `1px solid ${selectedDate ? "var(--amber-dim)" : "var(--border)"}`,
        ...
        <div style={{ marginBottom: 40 }}>
          <p style={{ fontFamily: "var(--sans)", fontSize: 11, color: "var(--text-dim)", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 6 }}>
            Last 7 Days
          </p>
          <p style={{ fontFamily: "var(--sans)", fontSize: 11, color: "var(--text-muted)", marginBottom: 16, letterSpacing: "0.04em" }}>
            Click a day to filter
          </p>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 88 }}>
            {days.map(({ label, total, date, count }) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{
                    ...
                    color: isSelected ? "var(--amber)" : "transparent",
                    letterSpacing: "0.06em", height: 12,
                  }}>{count > 0 ? count : ""}</span>
                  ...
                  <span style={{
                    fontFamily: "var(--mono)", fontSize: 9,
                    color: isSelected ? "var(--amber)" : isToday ? "var(--amber-dim)" : "var(--text-muted)",
        ...
        {Object.keys(byTag).length > 0 && (
          <div style={{ marginBottom: 40 }}>
            <p style={{ fontFamily: "var(--sans)", fontSize: 11, color: "var(--text-dim)", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16 }}>
              Time by Project {selectedDate ? `· ${statLabel}` : ""}
            </p>
                ...
                <div key={s.id} style={{
                  ...
                  <div>
                    ...
                    <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--text-muted)", marginLeft: 10 }}>
                      {new Date(s.ts).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
                      {!selectedDate && (
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 20 }}>
              <button
                ...
                style={{
                  ...
                  padding: "6px 16px", cursor: page === 1 ? "default" : "pointer",
                  fontFamily: "var(--mono)", fontSize: 11, letterSpacing: "0.1em",
                  color: page === 1 ? "var(--text-muted)" : "var(--text-dim)",
                  opacity: page === 1 ? 0.4 : 1, transition: "all 0.2s",
                }}
                onMouseEnter={e => { if (page !== 1) e.target.style.borderColor = "var(--amber)"; }}
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button key={i} onClick={() => setPage(i + 1)} style={{
                    ...
                    background: page === i + 1 ? "var(--amber)" : "var(--surface)",
                    color: page === i + 1 ? "#1a1208" : "var(--text-dim)",
                    transition: "all 0.2s",
                  }}>{i + 1}</button>
                ))}
              </div>
              ...
              <button
                ...
                style={{
                  background: "none", border: "1px solid var(--border)", borderRadius: 4,
                  padding: "6px 16px", cursor: page === totalPages ? "default" : "pointer",
                  fontFamily: "var(--mono)", fontSize: 11, letterSpacing: "0.1em",
                  color: page === totalPages ? "var(--text-muted)" : "var(--text-dim)",
                  opacity: page === totalPages ? 0.4 : 1, transition: "all 0.2s",
                }}
                onMouseEnter={e => { if (page !== totalPages) e.target.style.borderColor = "var(--amber)"; }}
                onMouseLeave={e => e.target.style.borderColor = "var(--border)"}
              >next →</button>
            </div>
          )}
...
function AuthScreen({ onAuth, onBack }) {
  ...
  const DEMO = { email: "demo@deepwork.app", password: "demo1234", name: "Alex" };
    ...
    setTimeout(() => {
      ...
      if (tab === "login") {
        if (email === DEMO.email && password === DEMO.password) {
          onAuth({ name: DEMO.name, email: DEMO.email });
        } else {
          setError("Invalid email or password.");
        }
      } else {
        // signup always succeeds in prototype
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* demo hint */}
        {tab === "login" && (
          <p style={{ marginTop: 16, textAlign: "center", fontFamily: "var(--sans)", fontSize: 11, color: "var(--text-muted)", letterSpacing: "0.04em" }}>
            Demo: <span
              style={{ color: "var(--amber-dim)", cursor: "pointer", textDecoration: "underline" }}
              onClick={() => { setEmail(DEMO.email); setPassword(DEMO.password); }}
            >use demo credentials</span>
          </p>
        )}
      </div>
...
export default function App() {
  ...
  if (view === "auth") return (
    <>
      <FontLoader />
      <AuthScreen onAuth={(u) => { setUser(u); setView("timer"); }} onBack={() => setView("timer")} />
    </>
  );
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <NavBtn onClick={() => {
              if (!user) { setView("auth"); } else { setView("analytics"); }
            }}>History</NavBtn>
            ...
            {user ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{
                  ...
                  fontFamily: "var(--mono)", fontSize: 11, color: "#1a1208", fontWeight: 500,
                }}>
                  {user.name[0].toUpperCase()}
                </div>
                <NavBtn onClick={() => { setUser(null); setView("timer"); setRunning(false); }}>Sign out</NavBtn>
              </div>
            ) : (
              <button onClick={() => setView("auth")} style={{
                ...
                color: "#1a1208", transition: "opacity 0.2s",
              }}
                onMouseEnter={e => e.target.style.opacity = "0.85"}
                onMouseLeave={e => e.target.style.opacity = "1"}
              >SIGN IN</button>
